/**
 * 模型摆放  更新版本   之前没考虑 多个企业  以及不造企业下的模型的  摆放情况  这一版本 都考虑进去
 */

/**
 * info  :包含多个企业数据  或者是多个单体模型的数据
 * viewer ：
 */
var UpdatePlacement = (function(){
    function _( info, viewer) {

        var instance = this;
        Cesium.Check.typeOf.object('info', info);
        ...
        //这个函数 主要是对摆放操作的封装  两个参数 第一个是  模型矩阵  第二个是企业下的所有模型 或者单体的单个模型
        function place(info_single , eq){
            Cesium.Check.typeOf.object('info_single', info_single);
            if(info_single.isCompany){//如果是企业
                var primities = info_single.models;
                info_single.newCompanyInfo = {
                    "orientation":{
                        x:undefined,
                        y:undefined,
                        z:undefined,
                ...
                var primity = primities[0];
                //因为后期 有个定位到初始位置和方位 所以这里应该再添加一个初始的矩阵和初始的位置属性
                var startInfo = getInitInfo(primity);
                info_single['modelMatrix_start'] = startInfo.matrix_s;
                info_single['start_position'] = startInfo.position_s;
                placeModelOrFirm(info_single);

            }else{//不是企业级别  即单个模型级别
                var models = info_single.models;
                Cesium.Check.typeOf.object('models', models);
                if(models.length <=0){return;}
                for(var i=0;i<models.length;i++){
                    var primity = models[i];
                    //判断模型类型
                    if(primity instanceof Cesium.Model){//promity加载方式
                        var model = primity;
                        model.newOrientation = {
                            x:undefined,
                            y:undefined,
                            z:undefined,
                            heading:undefined,
                        ...
                        var startInfo = getInitInfo(model);
                        model['modelMatrix_start'] = startInfo.matrix_s;
                        model['start_position'] = startInfo.position_s;
                        placeModelOrFirm(model);
                    }else if(primity instanceof  Cesium.Cesium3DTileset){//3dtileset类型的模型
                        var d3tileModel = primity;
                        d3tileModel.newOrientation = {
                            x:undefined,
                            y:undefined,
                            z:undefined,
                            heading:undefined,
                        ...
                        var startInfo = getInitInfo(d3tileModel);
                        d3tileModel['modelMatrix_start'] = startInfo.matrix_s;
                        d3tileModel['start_position'] = startInfo.position_s;
        ...
        //给企业或单个模型 附上初始化的一些  信息   参数必须是一个单一的模型
        function getInitInfo(single_model){
            Cesium.Check.typeOf.object('single_model', single_model);
            if("modelMatrix" in single_model){
                var primity = single_model;
                ...
                return {
                    "position_s":qiYP_strat,
                    "matrix_s":modelMatrix_start
                }
            }
        }

        //摆放模型或者企业
        function placeModelOrFirm(info){
            Cesium.Check.typeOf.object('info', info);
            var primity = undefined;
            var models = undefined;
            var startInfo= {};

            if(info.isCompany){//如果是企业
                primity = info['models'][0];
                models = info['models'];
                startInfo['startM'] = info['modelMatrix_start'];
                startInfo['startP'] = info['start_position'];
            }else{
                primity = info;
                models = [primity];
                startInfo['startM'] = primity['modelMatrix_start'];
                startInfo['startP'] = primity['start_position'];
            }
            var primities = models;
            if(!('modelMatrix' in primity)){return;}
            var modelMatrix = primity['modelMatrix'];
            var qiYP = new Cesium.Cartesian3;
            Cesium.Matrix4.getColumn(modelMatrix, 3, qiYP);
            ...
            viewer.clock.onTick.addEventListener(function (clock) {
                ...
                if (flags.shift) //如果点击shift的时候
                {
                    ...
                    if (bRotate && instance.start_place) {
                        ...
                        getQiYInfo(modelMatrix, qiYP, scene, info);
                ...
                var rightDir = new Cesium.Cartesian3(axes4.x, axes4.y, axes4.z);
                Cesium.Cartesian3.normalize(rightDir, rightDir);

                var at = new Cesium.Cartesian3();
            ...
            function getFlagForKeyCode(keyCode) {
                switch (keyCode) {
                    ...
                    case 32: {
                        if(!instance.start_place){return;}
                        //吧模型的模型矩阵恢复为初始值
                        var modelMatrix_start = startInfo['startM'];
                       var qiYP_strat = startInfo["startP"];
                        for (var i = 0; i < primities.length; i++) {
                            var primity = primities[i];
                        ...
                        getQiYInfo(modelMatrix_start, qiYP_strat, scene, info);
                        // //把相机定位到初始的位置
                        break;
                    }
                    case 'L'.charCodeAt(0): {
        ...
        function getQiYInfo(matrix,qiYP, scene, obj){
            var hpr = modelMatrixToHPR(matrix,scene);
            var LLH = wordToLLH(qiYP);
            if(obj.isCompany){
                obj.newCompanyInfo.heading = (hpr.heading*180)/Math.PI;
                obj.newCompanyInfo.pitch = (hpr.pitch*180)/Math.PI;
                obj.newCompanyInfo.roll = (hpr.roll*180)/Math.PI;
                obj.newCompanyInfo.x =LLH.lng;
                obj.newCompanyInfo.y = LLH.lat;
                obj.newCompanyInfo.z = LLH.alt;
            }else{
                obj.newOrientation.heading = (hpr.heading*180)/Math.PI;
                obj.newOrientation.pitch = (hpr.pitch*180)/Math.PI;
                obj.newOrientation.roll = (hpr.roll*180)/Math.PI;
                obj.newOrientation.x =LLH.lng;
                obj.newOrientation.y = LLH.lat;
                obj.newOrientation.z = LLH.alt;
            }
        }
